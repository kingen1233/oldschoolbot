...
import {
	defaultPatches,
	resolvePatchTypeSetting,
} from "../../lib/minions/farming";
...
export default class extends BotCommand {
	public constructor(store: CommandStore, file: string[], directory: string) {
		super(store, file, directory, {
			...
			usageDelim: " ",
			description:
				"Allows a player to harvest a specific patch without replanting a seed.",
	...
	async run(msg: KlasaMessage, [seedType = ""]: [string]) {
		...
		const currentWoodcuttingLevel = msg.author.skillLevel(
		...
		const patchType =
			msg.author.settings.get(getPatchType) ?? defaultPatches;
		...
		if (!planted)
			return msg.channel.send(
				`${msg.author}, you have nothing planted on this patch type.`
			);
		...
		let duration =
			...
			(timePerPatchTravel + timePerPatchHarvest) *
			xpBoost;
