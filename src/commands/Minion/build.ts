...
export default class extends BotCommand {
	...
	async run(
		...
		if (msg.flagArgs.xphr) {
			...
			for (let i = 1; i < 100; i += 5) {
				...
				for (const obj of Constructables) {
					...
					const xp = Math.floor(
						(Time.Hour / (obj.ticks * 300)) * obj.xp
					);
		...
		if (quantity === null) {
			...
			const maxForTime = Math.floor(
				maxTripLength / timeToBuildSingleObject
			);
			quantity = Math.floor(
				Math.min(maxForTime, Math.max(maxForMaterials, 1))
			);
		}
		...
		if (planksHas < planksQtyCost * quantity) {
			return msg.channel.send(
				`You don't have enough ${itemNameFromID(
					plank
				)} to make ${quantity}x ${object.name}.`
			);
		}
		...
		if (msg.author.settings.get(UserSettings.GP) < gpNeeded) {
			return msg.channel.send(
				"You don't have enough GP to pay your Butler."
			);
		}
		await msg.author.removeGP(gpNeeded);
		await msg.author.removeItemsFromBank(
			new Bank().add(plank, totalPlanksNeeded)
		);
