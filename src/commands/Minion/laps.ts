...
function alching(msg: KlasaMessage, tripLength: number) {
	...
	const hasInfiniteFireRunes = msg.author.hasItemEquippedAnywhere(
		unlimitedFireRuneProviders
	);
	...
	const bankToRemove = new Bank()
		.add("Nature rune", maxCasts)
		.add(itemToAlch.id, maxCasts);
	if (!hasInfiniteFireRunes) {
...
export default class extends BotCommand {
	...
	async run(
		...
		if (
			course.qpRequired &&
			msg.author.settings.get(UserSettings.QP) < course.qpRequired
		) {
			return msg.channel.send(
		...
		const alchResult =
			course.name === "Ape Atoll Agility Course"
				? null
				: alching(msg, duration);
		if (alchResult !== null) {
			if (!msg.author.owns(alchResult.bankToRemove)) {
				return msg.channel.send(
					`You don't own ${alchResult.bankToRemove}.`
				);
			}
