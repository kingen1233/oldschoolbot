...
export default class extends BotCommand {
	...
	async run(
		msg: KlasaMessage,
		[quantity, logName = ""]: [null | number | string, string]
	) {
		...
		const log = Firemaking.Burnables.find(
			(log) =>
				stringMatches(log.name, logName) ||
				stringMatches(log.name.split(" ")[0], logName)
		);
		...
		if (quantity === null) {
			const amountOfLogsOwned = msg.author.settings.get(
				UserSettings.Bank
			)[log.inputLogs];
		...
		const hasRequiredLogs = await msg.author.hasItem(
			log.inputLogs,
			quantity
		);
		if (!hasRequiredLogs) {
		...
		await msg.author.removeItemsFromBank(
			new Bank().add(log.inputLogs, quantity)
		);
