...
export default class extends BotCommand {
	...
	async run(
		...
		if (msg.flagArgs.json) {
			...
			if (json.length > 1900) {
				return msg.channel.send({
					files: [
						new MessageAttachment(Buffer.from(json), "bank.json"),
					],
				});
			}
		...
		if (msg.flagArgs.smallbank) {
			...
			const { uniqueSprite } = (
				this.client.tasks.get("bankImage") as BankImageTask
			).getBgAndSprite(userBg);
			const currentStatus = msg.author.settings
				.get(UserSettings.BitField)
				.includes(BitField.AlwaysSmallBank);
			...
			return msg.channel.send(
				`Small Banks are now ${
					...
					uniqueSprite
						? " You current BG will always draw the bank as small."
						: ""
				}`
			);
		}
		...
		if (msg.flagArgs.text) {
			...
			for (const [item, qty] of bank.items()) {
				if (
					...
					!item.name
						.toLowerCase()
						.includes(msg.flagArgs.search.toLowerCase())
				) {
					continue;
				}

				const filter = msg.flagArgs.filter
					? filterableTypes.find((type) =>
							type.aliases.some(
								(alias) => msg.flagArgs.filter === alias
							)
					  ) ?? null
					: null;
				...
				if (msg.flagArgs.id) {
					textBank.push(
						`${
							item.name
						} (${item.id.toString()}): ${qty.toLocaleString()}`
					);
				} else {
