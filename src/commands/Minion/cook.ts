...
export default class extends BotCommand {
	...
	async run(
		msg: KlasaMessage,
		[quantity, cookableName = ""]: [null | number | string, string]
	) {
		...
		if (
			cookable.id === itemID("Jug of wine") ||
			cookable.id === itemID("Wine of zamorak")
		) {
		...
		if (!userBank.fits(totalCost)) {
			return msg.channel.send(
				`You don't have enough items. You need: ${inputCost}.`
			);
		}
