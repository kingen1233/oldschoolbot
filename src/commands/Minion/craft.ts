...
export default class extends BotCommand {
	...
	async run(
		msg: KlasaMessage,
		[quantity, craftName = ""]: [null | number | string, string]
	) {
						...
						Buffer.from(
							Crafting.Craftables.map(
								(item) =>
									`${item.name} - lvl ${
										item.level
									} : ${Object.entries(item.inputItems)
										.map(
											(entry) =>
												`${entry[1]} ${itemNameFromID(
													parseInt(entry[0])
												)}`
										)
										.join(", ")}`
		...
		if (craftName.toLowerCase().includes("zenyte") && quantity === null)
			quantity = 1;

		const craftable = Crafting.Craftables.find(
			(item) =>
				stringMatches(item.name, craftName) ||
				item.alias?.some((a) => stringMatches(a, craftName))
		);
		...
		let timeToCraftSingleItem =
			craftable.tickRate * Time.Second * 0.6 + Time.Second / 4;
		const [hasFallyHard] = await userhasDiaryTier(
			msg.author,
			FaladorDiary.hard
		);
		if (
			craftable.bankChest &&
			(hasFallyHard || msg.author.skillLevel(SkillsEnum.Crafting) >= 99)
		) {
		...
		updateBankSetting(
			this.client,
			ClientSettings.EconomyStats.CraftingCost,
			itemsNeeded.bank
		);
