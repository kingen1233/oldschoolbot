...
export default class extends BotCommand {
	...
	async run(
		...
		if (msg.flagArgs.xphr) {
			let str =
				"Approximate XP/Hr and Food usage rates (varies based on RNG)\n\n";
			for (let i = 1; i < 100; i += 5) {
				...
				for (const npc of Pickpocketables) {
					...
					results.push([
						npc.name,
						round(xpReceived, 2) / 5,
						damageTaken / 5,
					]);
				}
				for (const [name, xp, damageTaken] of results.sort(
					(a, b) => a[1] - b[1]
				)) {
		...
		const pickpocketable = Pickpocketables.find((npc) =>
			stringMatches(npc.name, name)
		);
		...
		const [hasArdyHard] = await userhasDiaryTier(
			msg.author,
			ArdougneDiary.hard
		);
		if (hasArdyHard) {
		...
		const [successfulQuantity, damageTaken, xpReceived] =
			calcLootXPPickpocketing(
