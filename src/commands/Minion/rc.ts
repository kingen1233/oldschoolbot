...
export default class extends BotCommand {
	...
	async run(
		msg: KlasaMessage,
		[quantity, name = ""]: [null | number | string, string]
	) {
		...
		const rune = Runecraft.Runes.find(
			(_rune) =>
				stringMatches(_rune.name, name) ||
				stringMatches(_rune.name.split(" ")[0], name)
		);
		...
		if (
			rune.qpRequired &&
			msg.author.settings.get(UserSettings.QP) < rune.qpRequired
		) {
			return msg.channel.send(
				`You need ${rune.qpRequired} QP to craft this rune.`
			);
		}

		const numEssenceOwned = await msg.author.numberOfItemInBank(
			itemID("Pure essence")
		);
		...
		if (msg.flagArgs.ns) {
			...
			msg.author.hasItemEquippedOrInBank(
		...
		for (const pouch of Runecraft.pouches) {
			if (msg.author.skillLevel(SkillsEnum.Runecraft) < pouch.level)
				break;
		...
		const numberOfInventories = Math.max(
			Math.ceil(quantity / inventorySize),
			1
		);
		...
		if (rune.inputTalisman) {
			if (msg.flagArgs.talisman) {
				removeTalismanAndOrRunes.add(
					rune.inputTalisman.clone().multiply(numberOfInventories)
				);
				...
				const tomeOfFire = msg.author.hasItemEquippedAnywhere([
					"Tome of fire",
					"Tome of fire (empty)",
				])
					? 0
					: 7;
				const tomeOfWater = msg.author.hasItemEquippedAnywhere([
					"Tome of water",
					"Tome of water (empty)",
				])
					? 0
				...
				removeTalismanAndOrRunes.add(
					determineRunes(
						...
						new Bank({
							"Astral rune": 2,
							"Fire rune": tomeOfFire,
							"Water rune": tomeOfWater,
						})
							.clone()
							.multiply(numberOfInventories)
					)
			...
			if (!msg.author.bank().has(removeTalismanAndOrRunes.bank)) {
				return msg.channel.send(
					`You don't have enough runes for this trip. You need ${rune.inputRune
						?.clone()
						.multiply(quantity)}.`
				);
			}
			removeTalismanAndOrRunes.add(
				"Binding necklace",
				Math.max(Math.floor(numberOfInventories / 8), 1)
			);
			if (!msg.author.bank().has(removeTalismanAndOrRunes.bank)) {
				return msg.channel.send(
		...
		updateBankSetting(
			this.client,
			ClientSettings.EconomyStats.RunecraftCost,
			totalCost
		);
		...
		let response = `${
			msg.author.minionName
		} is now turning ${quantity}x Essence into ${
			rune.name
