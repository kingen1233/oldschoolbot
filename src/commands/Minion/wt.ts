...
export default class extends BotCommand {
	public constructor(store: CommandStore, file: string[], directory: string) {
		super(store, file, directory, {
			...
			description:
				"Sends your minion to fight the Wintertodt. Requires food and warm items.",
			examples: ["+wt"],
		});
	}
	...
	async run(msg: KlasaMessage) {
		...
		if (fmLevel < 50) {
			return msg.channel.send(
				"You need 50 Firemaking to have a chance at defeating the Wintertodt."
			);
		}

		const messages = [];

		let durationPerTodt = Time.Minute * 7.3 * 0.5;

		// Up to a 10% boost for 99 WC
		const wcBoost = (wcLevel + 1) / 10;
		if (wcBoost > 1)
			messages.push(`${wcBoost.toFixed(2)}% boost for Woodcutting level`);
		durationPerTodt =
			reduceNumByPercent(durationPerTodt, wcBoost) * xpBoost;
		...
		durationPerTodt = reduceNumByPercent(
			durationPerTodt,
			5 * warmGearAmount
		);
		...
		for (const food of Eatables) {
			const healAmount =
				typeof food.healAmount === "number"
					? food.healAmount
					: food.healAmount(msg.author);
			const amountNeeded =
				Math.ceil(healAmountNeeded / healAmount) * quantity;
			...
			messages.push(
				`Removed ${amountNeeded}x ${food.name}'s from your bank`
			);
