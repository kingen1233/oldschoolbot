...
function soloMessage(user: KlasaUser, duration: number, quantity: number) {
	const kc =
		user.settings.get(UserSettings.MonsterScores)[NightmareMonster.id] ?? 0;
	let str = `${user.minionName} is now off to kill The Nightmare ${quantity} times.`;
	if (kc < 5) {
		str +=
			" They are terrified to face The Nightmare, and set off to fight it with great fear.";
	} else if (kc < 10) {
		str +=
			" They are scared to face The Nightmare, but set off with great courage.";
	} else if (kc < 50) {
		str +=
			" They are confident in killing The Nightmare, and prepared for battle.";
	} else {
		str +=
			" They are not scared of The Nightmare anymore, and ready to fight!";
	}

	return `${str} The trip will take approximately ${formatDuration(
		duration
	)}.`;
}
...
export default class extends BotCommand {
	...
	checkReqs(users: KlasaUser[], monster: KillableMonster, quantity: number) {
		...
		for (const user of users) {
			...
			if (
				!hasEnoughFoodForMonster(monster, user, quantity, users.length)
			) {
				throw `${
					users.length === 1
						? "You don't"
	...
	async run(
		msg: KlasaMessage,
		[type, maximumSizeForParty]: ["mass" | "solo", number]
	) {
		...
		const partyOptions: MakePartyOptions = {
			...
			customDenier: (user) => {
				...
				const [hasReqs, reason] =
					user.hasMonsterRequirements(NightmareMonster);
				if (!hasReqs) {
					return [
						true,
		...
		duration =
			quantity * perKillTime * xpBoost - NightmareMonster.respawnTime!;

		const totalCost = new Bank();
		if (NightmareMonster.healAmountNeeded) {
			for (const user of users) {
				const [healAmountNeeded] = calculateMonsterFood(
					NightmareMonster,
					user
				);
