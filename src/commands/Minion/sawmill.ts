...
export default class extends BotCommand {
	...
	async run(
		msg: KlasaMessage,
		[quantity, plankName = ""]: [null | number | string, string]
	) {
		...
		if (quantity === 0) {
			return msg.channel.send(
				`You don't have any ${itemNameFromID(plank.inputItem)}.`
			);
		}
		...
		if (GP < cost) {
			return msg.channel.send(
				`You need ${toKMB(cost)} GP to create ${quantity} planks.`
			);
		}
		...
		await msg.author.removeItemsFromBank(
			new Bank().add(plank!.inputItem, quantity)
		);
		...
		let response = `${
			msg.author.minionName
		} is now creating ${quantity} ${itemNameFromID(plank.outputItem)}${
			quantity > 1 ? "s" : ""
		}. The Sawmill has charged you ${toKMB(
			cost
