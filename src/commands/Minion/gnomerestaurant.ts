	...
	async run(msg: KlasaMessage) {
		...
		if (gp < 5000) {
			return msg.channel.send(
				"You need atleast 5k GP to work at the Gnome Restaurant."
			);
		}
		...
		const duration =
			randomVariation(deliveryLength * quantity, 5) * xpBoost;

		if (msg.author.skillLevel(SkillsEnum.Magic) >= 66) {
			itemsToRemove.add(
				"Law rune",
				Math.max(1, Math.floor(randInt(1, quantity * 1.5) / 2))
			);
		}

		if (!msg.author.owns(itemsToRemove)) {
			return msg.channel.send(
				`You don't own the required items: ${itemsToRemove}.`
			);
		}
