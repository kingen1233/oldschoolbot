...
export default class extends BotCommand {
	...
	async run(msg: KlasaMessage, [type]: [string | undefined]) {
				...
				...(await Promise.all(
					[1, 2, 3, 4, 5, 6, 7, 8, 9, 15].map(async (i) => {
						...
						return [
							ar.length,
							formatDuration(
								(await calcCoxDuration(ar, false)).duration
							),
							(
								await createTeam(ar, false)
							)[0].personalPoints.toLocaleString(),
							`${(await createTeam(ar, false))[0].deathChance}%`,
							formatDuration(
								(await calcCoxDuration(ar, true)).duration
							),
							(
								await createTeam(ar, true)
							)[0].personalPoints.toLocaleString(),
		...
		if (!type) {
			...
			const totalPoints = msg.author.settings.get(
			...
			const normalTeam = await calcCoxDuration(
				Array(2).fill(msg.author),
				false
			);
			const cmSolo = await calcCoxDuration([msg.author], true);
			const cmTeam = await calcCoxDuration(
				Array(2).fill(msg.author),
				true
			);
			return msg.channel
				.send(`<:bow_and_arrow:847736079909519371> Chamber's of Xeric <:Olmlet:324127376873357316>
...
**Gear Score:** ${Emoji.Gear}${total.toFixed(1)}%`);
		...
		if (type === "gear") {
			const { melee, range, mage, total } = calculateUserGearPercents(
				msg.author
			);
			return msg.channel
				.send(`**Melee Gear Score:** <:crossed_swords:847735998356258816> ${melee.toFixed(
				1
			)}%
**Range Gear Score:** <:bow_and_arrow:847736079909519371> ${range.toFixed(1)}%
**Mage Gear Score:** <:man_mage:847736195190489095> ${mage.toFixed(1)}%
...
**Death Chance Team:** ${Emoji.Skull} ${(
		...
		if (type !== "mass" && type !== "solo") {
			return msg.send(
				`Specify your team setup for Chamber's of Xeric, either solo or mass.`
			);
		}
		...
		*/
		...
		const partyOptions: MakePartyOptions = {
			...
			customDenier: (user) => {
				...
				if (!hasMinRaidsRequirements(user)) {
					return [
						true,
						`You meet the stat requirements to do the Chamber's of Xeric.`,
					];
				}
		...
		const users =
			type === "mass"
				? (await msg.makePartyAwaiter(partyOptions)).filter(
						(u) => !u.minionIsBusy
				  )
				: [msg.author];
		...
		let { duration, totalReduction, reductions } = await calcCoxDuration(
			users,
			isChallengeMode
		);
		...
		const amountOfRaids = 50;
		for (let i = 0; i < amountOfRaids; i++) {
			await Promise.all(
				users.map(async (u) => {
					...
					debugStr += `${u.username} (${Emoji.Gear}${total.toFixed(
						1
		...
		let str = isSolo
			? `${
			  ...
			  } is now doing alot of Chamber's of Xeric raids. The total trip will take ${formatDuration(
					duration
			  )}.`
