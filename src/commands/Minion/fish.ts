	...
	async run(
		...
		if (fish.qpRequired) {
			if (msg.author.settings.get(UserSettings.QP) < fish.qpRequired) {
				return msg.channel.send(
					`You need ${fish.qpRequired} qp to catch those!`
				);
			}
		}
		...
		if (
			...
			return msg.channel.send(
				"You need at least 15 Agility and Strength to do Barbarian Fishing."
			);
		}
		...
		if (fish.bait) {
			...
			if (!hasBait) {
				return msg.channel.send(
					`You need ${itemNameFromID(fish.bait)} to fish ${
						fish.name
					}!`
				);
			}
		...
		if (fish.bait) {
			await msg.author.removeItemsFromBank(
				new Bank().add(fish.bait, quantity)
			);
		}
