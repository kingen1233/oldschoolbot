...
export default class extends BotCommand {
	...
	async run(msg: KlasaMessage, [input]: [string]) {
		...
		const quantity = Math.floor(
			msg.author.maxTripLength(Activity.SoulWars) / perDuration
		);
		...
		if (input === "solo") {
			...
			const users = (await msg.makePartyAwaiter(partyOptions)).filter(
				(u) => !u.minionIsBusy
			);
			if (users.length === 0) {
				return;
		...
		return msg.channel.send(
			`Commands: +sw solo/mass/buy/imbue. Current SW points: ${msg.author.settings.get(
	...
	async buy(msg: KlasaMessage, [input = ""]: [string]) {
		...
		if (bal < item.tokens * quantity) {
			return msg.channel.send(
				`You don't have enough Zeal Tokens to buy ${quantity} ${
					item.item.name
				}. You have ${bal} but need ${item.tokens * quantity}.`
			);
		}
		await msg.author.settings.update(
			UserSettings.ZealTokens,
			bal - item.tokens * quantity
		);
		await msg.author.addItemsToBank({ [item.item.id]: quantity }, true);
		return msg.channel.send(
			`Added ${quantity}x ${item.item.name} to your bank, removed ${
	...
	async imbue(msg: KlasaMessage, [input = ""]: [string]) {
		const item = imbueables.find(
			(i) =>
				stringMatches(input, i.input.name) ||
				stringMatches(input, i.output.name)
		);
		if (!item) {
			return msg.channel.send(
