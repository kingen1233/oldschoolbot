...
export default class extends BotCommand {
	...
	async run(
		msg: KlasaMessage,
		[quantity, mixName = ""]: [null | number | string, string]
	) {
		...
		let timeToMixSingleItem =
			mixableItem.tickRate * Time.Second * 0.6 +
			mixableItem.bankTimePerPotion * Time.Second;
		...
		if (zahur && mixableItem.zahur === true) {
			...
			cost =
				"decided to pay Zahur 200 gp for each potion so they don't have to go";
		}
		if (msg.flagArgs.wesley && mixableItem.wesley === true) {
			timeToMixSingleItem = 0.000_001;
			requiredItems.add("Coins", 50);
			cost =
				"decided to pay Wesley 50 gp for each item so they don't have to go";
		}
		...
		if (maxCanDo === 0) {
			return msg.channel.send(
				"You don't have enough supplies to mix even one of this item!"
			);
		}
