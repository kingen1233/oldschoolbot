...
export default class extends BotCommand {
	...
	async run(
		msg: KlasaMessage,
		[quantity, smithableItem = ""]: [null | number | string, string]
	) {
		...
		if (maxCanDo === 0) {
			return msg.channel.send(
				"You don't have enough supplies to smith even one of this item!"
			);
		}
		...
		return msg.channel.send(
			`${msg.author.minionName} is now smithing ${
				quantity * smithedItem.outputMultiple
			}x ${
				smithedItem.name
			}, removed ${cost} from your bank, it'll take around ${formatDuration(
				duration
			)} to finish.`
		);
	}
