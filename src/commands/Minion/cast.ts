...
export default class extends BotCommand {
	...
	async run(
		...
		const spell = Castables.find((spell) =>
			stringMatches(spell.name, name)
		);
		...
		if (
			spell.craftLevel &&
			msg.author.skillLevel(SkillsEnum.Crafting) < spell.craftLevel
		) {
			return msg.channel.send(
		...
		if (
			spell.qpRequired &&
			msg.author.settings.get(UserSettings.QP) < spell.qpRequired
		) {
			return msg.channel.send(
		...
		let timeToEnchantTen =
			spell.ticks * Time.Second * 0.6 + Time.Second / 4;
		...
		if (duration > maxTripLength) {
			return msg.channel.send(
				`${
					...
					spell.name
				}s you can cast is ${Math.floor(
					maxTripLength / timeToEnchantTen
				)}.`
			);
		}
		const cost = determineRunes(
			msg.author,
			spell.input.clone().multiply(quantity)
		);
		if (!userBank.has(cost.bank)) {
			return msg.channel.send(
				`You don't have the materials needed to cast ${quantity}x ${
					spell.name
				}, you need ${spell.input}, you're missing **${cost
					.clone()
					.remove(userBank)}** (Cost: ${cost}).`
			);
		}
		...
		if (spell.gpCost) {
			...
			if (gpCost > userGP) {
				return msg.channel.send(
					`You need ${gpCost} GP to create ${quantity} planks.`
				);
			}
