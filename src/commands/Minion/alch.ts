...
export default class extends BotCommand {
	public constructor(store: CommandStore, file: string[], directory: string) {
		super(store, file, directory, {
			...
			oneAtTime: true,
			description:
				"Allows you to send your minion to alch items from your bank",
	...
	async run(
		msg: KlasaMessage,
		[quantity = null, item]: [number | null, Item[]]
	) {
		const userBank = msg.author.bank();
		let osItem = item?.find(
			(i) => userBank.has(i.id) && i.highalch && i.tradeable
		);
		...
		if (msg.author.skillLevel(SkillsEnum.Magic) < 55) {
			return msg.channel.send(
				"You need level 55 Magic to cast High Alchemy"
			);
		}
		...
		const maxCasts = Math.min(
			Math.floor(maxTripLength / timePerAlch),
			userBank.amount(osItem.id)
		);
		...
		if (!msg.author.owns(consumedItems)) {
			return msg.channel.send(
				`You don't have the required items, you need ${consumedItems}`
			);
		}

		await msg.confirm(
			`${msg.author}, please confirm you want to alch ${quantity} ${
				osItem.name
			} (${Util.toKMB(
				alchValue
		...
		msg.author.log(
			`alched Quantity[${quantity}] ItemID[${osItem.id}] for ${alchValue}`
		);

		const response = `${
			msg.author.minionName
		} is now alching ${quantity}x ${
			osItem.name
