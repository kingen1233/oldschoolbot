...
export default class extends BotCommand {
	...
	async run(
		...
		for (const [itemID, boostAmount] of Object.entries(
			...
			boosts.push(
				`${boostAmount}% for ${itemNameFromID(parseInt(itemID))}`
			);
		}
		...
		const salveBoost = boosts
			.join("")
			.toLowerCase()
			.includes("salve amulet");
		if (!salveBoost) {
		...
		if (
			...
			return msg.channel.send(
				`${monster!.name} cannot be barraged or burst.`
			);
		}
		if (msg.flagArgs.cannon && !hasCannon) {
			return msg.channel.send(
				"You don't own a Dwarf multicannon, so how could you use one?"
			);
		}
		if (msg.flagArgs.cannon && !monster!.canCannon) {
			return msg.channel.send(
				`${monster!.name} cannot be killed with a cannon.`
			);
		}
		if (
			boostChoice === "barrage" &&
			msg.author.skillLevel(SkillsEnum.Magic) < 94
		) {
			return msg.channel.send(
				`You need 94 Magic to use Ice Barrage. You have ${msg.author.skillLevel(
					SkillsEnum.Magic
				)}`
			);
		}
		if (
			boostChoice === "burst" &&
			msg.author.skillLevel(SkillsEnum.Magic) < 70
		) {
			return msg.channel.send(
				`You need 70 Magic to use Ice Burst. You have ${msg.author.skillLevel(
					SkillsEnum.Magic
				)}`
			);
		}
		...
		const infiniteWaterRunes = msg.author.hasItemEquippedAnywhere(
			...
			for (const cc of consumableCosts) {
				...
				if (itemMultiple) {
					...
					if (cc.itemCost) {
						...
						const oneKcCost = cc.itemCost
							.clone()
							.multiply(multiply);
						// Can't use Bank.add() because it discards < 1 qty.
						for (const [itemID, qty] of objectEntries(
							oneKcCost.bank
						)) {
