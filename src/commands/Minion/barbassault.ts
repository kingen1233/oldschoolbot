...
export default class extends BotCommand {
	public constructor(store: CommandStore, file: string[], directory: string) {
		super(store, file, directory, {
			...
			description:
				"Sends your minion to do barbarian assault, or buy rewards and gamble.",
	...
	async run(msg: KlasaMessage) {
		return msg.channel.send(
			...
			)} **High Gambles:** ${msg.author.settings.get(
	...
	async level(msg: KlasaMessage) {
		...
		if (currentLevel === 5) {
			return msg.channel.send(
				"You've already reached the highest possible Honour level."
			);
		}
	...
	async buy(msg: KlasaMessage, [qty = 1, input = ""]: [number, string]) {
		const buyable = BarbBuyables.find((i) =>
		...
		if (balance < cost * qty) {
			return msg.channel.send(
				`You don't have enough Honour Points to buy ${qty.toLocaleString()}x ${
					item.name
				}. You need ${(
					cost * qty
		...
		await msg.confirm(
			`Are you sure you want to buy ${qty.toLocaleString()}x ${
				item.name
			}, for ${(cost * qty).toLocaleString()} honour points?`
		);
		await msg.author.settings.update(
			UserSettings.HonourPoints,
			balance - cost * qty
		...
		return msg.channel.send(
			`Successfully purchased ${qty.toLocaleString()}x ${
				item.name
			} for ${(cost * qty).toLocaleString()} Honour Points.`
		);
	}
	...
	async gamble(msg: KlasaMessage, [qty = 1, tier = ""]: [number, string]) {
		...
		if (loot.has("Pet penance queen")) {
			...
			this.client.emit(
				...
				`<:Pet_penance_queen:324127377649303553> **${
					msg.author.username
				}'s** minion, ${
					msg.author.minionName
				}, just received a Pet penance queen from their ${formatOrdinal(
					gamblesDone
				)} High gamble! They are the ${formatOrdinal(
					countUsersHas
				)} to it.`
			);
		}
	...
	async start(msg: KlasaMessage, [qty = 0, input]: [number, string]) {
		const partyOptions: MakePartyOptions = {
			...
			message: `${
				msg.author.username
			} has created a Barbarian Assault party${
				qty > 0
					? ` for a maximum of ${qty} wave${qty > 1 ? "s" : ""}`
					: ""
		...
		const users =
			input === "solo"
				? [msg.author]
				: await msg.makePartyAwaiter(partyOptions);
		...
		const totalLevelPercent = round(
			calcWhatPercent(totalLevel, 5 * users.length) / 3.3,
			2
		);
		...
		const averageKC =
			addArrayOfNumbers(
				await Promise.all(
					users.map((u) => u.getMinigameScore("BarbarianAssault"))
				)
			) / users.length;
		const kcPercent = round(
			Math.min(100, calcWhatPercent(averageKC, 200)) / 5,
			2
		);
		...
		const quantity = Math.floor(
			msg.author.maxTripLength(Activity.BarbarianAssault) / waveTime
		);
